...
function renderArticleTree() {
  ...
  const items = headings.map((heading, index) => {
    ...
    return { id: heading.id, level: Number(heading.tagName.slice(1)), title: heading.textContent.trim() };
  });
  tree.innerHTML = `
...
async function renderLinkPreviews() {
  ...
  await Promise.all(links.map(async (link) => {
    ...
    if (isLocalArticle) {
      ...
      if (link.dataset.previewDescription) appendPreviewDescription(card, link.dataset.previewDescription);
      return;
    }
    try {
      ...
      if (preview.description) appendPreviewDescription(card, preview.description);
      if (preview.image) {
...
async function convertExistingQiitaArticleLinks() {
...
function appendPreviewDescription(card, text) {
...
function observeArticleTree(headings, tree) {
  ...
  const links = new Map([...tree.querySelectorAll("a[href^='#']")].map((link) => [decodeURIComponent(link.hash.slice(1)), link]));
