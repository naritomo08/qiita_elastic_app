...
フロントエンドは静的HTML/CSS/JavaScript SPA、バックエンドは独立したREST APIです。両者はJSON APIだけで接続し、6言語の実装を同じ画面から切り替えられます。ブランチを切り替えることで、frontendに適用するフレームワークも切り替えられます。
...
| `omake/readme4.md` | 自宅 HDFS の Iceberg テーブル `nginx_access_curated` を AWS 側へ日付単位で複製し、Athena や QuickSight / Quick から参照する手順。指定日または今回追加した AWS 側データだけを削除する手順も含みます。 |
